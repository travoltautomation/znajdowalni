...
function validate(body) {
  ...
  const limits = {email:254,source:1500,company:150,city:100,message:3000,website:200,industry:100,requestId:36,utm_source:100,utm_medium:100,utm_campaign:200};
  const data = {};
  ...
  } else if (!data.company || !data.city) return {error:'Podaj link albo nazwę firmy i miejscowość.',field:!data.company?'company':'city'};
...
function record(data, test = false) {
  return {schemaVersion:1,id:data.requestId,status:'new',test,submittedAt:new Date().toISOString(),contact:{email:data.email},business:{source:data.source,company:data.company,city:data.city,industry:data.industry},message:data.message,campaign:{source:data.utm_source,medium:data.utm_medium,name:data.utm_campaign},consent:{contact:true,policyVersion:'2026-09-07'},research:{sources:[],facts:[],toConfirm:[],direction:null}};
}
